...
export async function generateMetadata(): Promise<Metadata> {
    ...
    const ui = getTranslatedUI(locale, 'mirror-text');
    const metaTitle = ui?.name || 'Upside Down & Mirror Text Generator — Flip Your Text';
    const metaDesc = ui?.heroText || 'Flip your text upside down or mirror it instantly! Works on WhatsApp, Instagram, Discord, TikTok & more. Free, no sign-up needed. Just type and copy!';
...
export default async function Layout({ children }: { children: React.ReactNode }) {
    const { locale, dictionary: t } = await getLocaleDictionary();
    const ui = getTranslatedUI(locale, 'mirror-text');
    const displayName = ui?.name || 'Upside Down & Mirror Text Generator — Flip Your Text';
    ...
    const breadcrumbSchema = {
        ...
        itemListElement: [
            ...
            { '@type': 'ListItem', position: 3, name: displayName, item: `${SITE_URL}/tool/mirror-text` },
        ],
    };
    ...
    const webPageSchema = {
        ...
        '@type': 'WebPage',
        name: displayName,
        description: ui?.heroText || 'Flip your text upside down or mirror it instantly! Works on WhatsApp, Instagram, Discord, TikTok & more. Free, no sign-up needed. Just type and copy!',
        url: `${SITE_URL}/tool/mirror-text`,
