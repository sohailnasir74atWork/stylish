...
export async function generateMetadata(): Promise<Metadata> {
    ...
    const ui = getTranslatedUI(locale, 'captions');
    const metaTitle = ui?.name || 'Captions & Status Ideas — 165+ Ready to Copy for Instagram & WhatsApp';
    const metaDesc = ui?.heroText || 'Browse 165+ captions & status ideas for Instagram, WhatsApp, TikTok & Facebook. Attitude, love, sad, funny, gaming, birthday & more. Tap to copy!';
...
export default async function Layout({ children }: { children: React.ReactNode }) {
    const { locale, dictionary: t } = await getLocaleDictionary();
    const ui = getTranslatedUI(locale, 'captions');
    const displayName = ui?.name || 'Captions & Status Ideas — 165+ Ready to Copy for Instagram & WhatsApp';
    ...
    const breadcrumbSchema = {
        ...
        itemListElement: [
            ...
            { '@type': 'ListItem', position: 3, name: displayName, item: `${SITE_URL}/tool/captions` },
        ],
    };
    ...
    const webPageSchema = {
        ...
        '@type': 'WebPage',
        name: displayName,
        description: ui?.heroText || 'Browse 165+ captions & status ideas for Instagram, WhatsApp, TikTok & Facebook. Attitude, love, sad, funny, gaming, birthday & more. Tap to copy!',
        url: `${SITE_URL}/tool/captions`,
